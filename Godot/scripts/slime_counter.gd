...
func _ready() -> void:
    # Crée une zone de HUD simple au runtime (top center)
    _root_control = Control.new()
    _root_control.name = "SlimeCounterControl"
    _root_control.h_size_flags = Control.SIZE_EXPAND_FILL
    _root_control.v_size_flags = Control.SIZE_FILL
    ...
    _big_label.align = Label.ALIGN_CENTER
    _big_label.valign = Label.VALIGN_CENTER
...
func _show_count(count: int) -> void:
    ...
    _label.visible = true
    _label.modulate = Color(1,1,1,0)
    _label.rect_scale = Vector2(0.9, 0.9)

    var tween = get_tree().create_tween()
    tween.tween_property(_label, "modulate:a", 1.0, 0.18).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_OUT)
    tween.tween_property(_label, "rect_scale", Vector2(1.05,1.05), 0.18).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_OUT)
    tween.tween_callback(Callable(self, "_hold_and_fade"), 1.2)

    # Si objectif atteint, afficher le message de félicitations
    if count >= GOAL:
        _show_goal_message()
...
func _hold_and_fade() -> void:
    ...
    tween.tween_callback(Callable(self, "_hide_label"), 0.7)
...
func _show_goal_message() -> void:
    ...
    _big_label.modulate = Color(1,1,1,0)
    _big_label.rect_scale = Vector2(0.9,0.9)

    var tween = get_tree().create_tween()
    tween.tween_property(_big_label, "modulate:a", 1.0, 0.25).set_trans(Tween.TRANS_BACK).set_ease(Tween.EASE_OUT)
    tween.tween_property(_big_label, "rect_scale", Vector2(1.08,1.08), 0.25)
    tween.tween_callback(Callable(self, "_hold_and_hide_big"), 2.0)

func _hold_and_hide_big() -> void:
    var tween = get_tree().create_tween()
    tween.tween_property(_big_label, "modulate:a", 0.0, 0.6).set_delay(0.1)
    tween.tween_callback(Callable(self, "_hide_big_label"), 0.7)
